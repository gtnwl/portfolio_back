...
//package.json "devDependencies" 다음 라인에 "browserslist": ["ie > 0", "chrome > 0", "firefox > 0"] 추가
...
const paths = {
    html: {
        src: [`${src}**/*.html`, `!${src}**/include/*.html`, `!${src}**/unit/*.html`, `!${src}**/pop/*.html`,],
        dist: [`${src}**/*.html`, `!${src}**/include/*.html`, `!${src}**/unit/*.html`, `!${src}**/pop/*.html`, `!${src}**/sample/*.html`]
    },
    css: {
...
async function css() {
    ...
    gulp.src(paths.css.scss)
        ...
        //.pipe(gulpif(buildType, autoprefixer({browsers: ['chrome > 0', 'firefox > 0', 'ie > 0']})))
        .pipe(rename({
            prefix: prefix,
        }))
...
async function bfj() {
    ...
    gulp.src([`${distPath}**/*.js`, `!${distPath}**/js/lib/*.js`])
